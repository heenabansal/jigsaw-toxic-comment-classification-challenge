...
from keras.layers import Input, Embedding, SpatialDropout1D,LSTM, GRU,Dense, Masking, Bidirectional, GlobalAveragePooling1D, GlobalMaxPooling1D, RepeatVector, Dropout, merge,TimeDistributed, Flatten, Permute, Lambda
...
def get_model(maxlen_input, lstm_size,embedding_matrix,dense1_size):
    ...
    x = Bidirectional(GRU(lstm_size, return_sequences=True))(x)
...
lrate = LearningRateScheduler(step_decay)  #not working
...
sample_submission.to_csv("gru_baseline.csv", index=False)
